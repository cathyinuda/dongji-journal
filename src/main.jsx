...
const COLORS=['#0A84FF','#FFD60A','#BF5AF2','#30D158','#FF9F0A','#FF453A','#FF375F','#8E8E93'];
const DEFAULT_PROJECTS=[{id:'glutes',name:'臀腿',color:COLORS[0]},{id:'climb',name:'爬坡',color:COLORS[1]},{id:'upper',name:'上肢',color:COLORS[2]}];
...
function App(){
 ...
 const[projects,setProjects]=useState(()=>demo?DEFAULT_PROJECTS:read(PROJECT_KEY,DEFAULT_PROJECTS));
 ...
 const importBackup=async event=>{const file=event.target.files?.[0];if(!file)return;try{const data=JSON.parse(await file.text());if(!Array.isArray(data.projects)||!data.entries||typeof data.entries!=='object')throw new Error();setProjects(data.projects);setEntries(data.entries);persist(data.projects,data.entries);alert('备份已恢复')}catch{alert('无法读取这个备份文件')}event.target.value=''};
